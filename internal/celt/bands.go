...
func quantBand(
	...
	if !stereo && level == 0 {
		...
		if originalBlocks > 1 {
			if lowband != nil {
				deinterleaveHadamard(lowband, nPerBlock>>recombine, originalBlocks<<recombine, longBlocks)
			}
		}
	}
	...
	if split {
		...
		*remainingBits -= currentBits
		...
		if q != 0 {
			collapseMask = algUnquant(x, n, getPulses(q), spread, blocks, state.rangeDecoder, gain)
		} else {
	...
	if stereo {
		...
		x = fullBand
		if originalBlocks > 1 {
			interleaveHadamard(x, nPerBlock>>recombine, originalBlocks<<recombine, longBlocks)
		}
...
func deinterleaveHadamard(x []float32, n0 int, stride int, hadamard bool) {
...
func interleaveHadamard(x []float32, n0 int, stride int, hadamard bool) {
	tmp := make([]float32, n0*stride)
	if hadamard {
		ordery := orderyTable[stride-2:]
		for i := range stride {
