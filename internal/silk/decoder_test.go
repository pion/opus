...
func TestResetPredictionForBandwidthChange(t *testing.T) {
	...
	decoder.fixedSLPCQ14[0] = 1
	decoder.fixedOutBuf[0] = 1
	...
	assert.Equal(t, int32(1), decoder.fixedSLPCQ14[0])
	assert.Equal(t, int16(1), decoder.fixedOutBuf[0])
	...
	assert.Equal(t, [maxPredictLPCOrder]int32{}, decoder.fixedSLPCQ14)
	assert.Equal(t, [maxFrameLength + 2*maxSubFrameLength]int16{}, decoder.fixedOutBuf)
...
func TestStereoUnmixFixed(t *testing.T) {
	decoder := NewDecoder()
	mid := make([]float32, 80)
	side := make([]float32, len(mid))
	out := make([]float32, 2*len(mid))
	mid[0] = 0.5
	mid[len(mid)-1] = 0.25

	decoder.stereoUnmixFixed(mid, side, out, 0, 0, BandwidthNarrowband)

	expected := make([]float32, len(out))
	expected[2], expected[3] = 0.5, 0.5
	assert.Equal(t, expected, out)
	assert.Equal(t, [2]float32{0, 0.25}, decoder.previousMidValues)
	assert.True(t, decoder.wasStereo)

	clear(mid)
	decoder.stereoUnmixFixed(mid, side, out, 0, 0, BandwidthNarrowband)
	clear(expected)
	expected[0], expected[1] = 0.25, 0.25
	assert.Equal(t, expected, out)
}
