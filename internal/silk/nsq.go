...
const (
	...
	// maxPredictLPCOrder matches libopus's MAX_LPC_ORDER (define.h) — the
	// encoder's own max prediction order, distinct from maxLPCOrder
	// (SILK_MAX_ORDER_LPC=24, SigProc_FIX.h) used by the Schur/k2a analysis
	// math in lpc_float.go. PredCoef_Q12 is laid out in strides of this size.
	maxPredictLPCOrder = 16

	nsqLPCBufLength     = maxPredictLPCOrder
	harmShapeFIRTaps    = 3
...
type nsqParams struct {
	predCoefQ12      []int16 // 2*maxPredictLPCOrder
...
func (nsq *nsqState) quantize(x16 []int16, pulses []int8, p *nsqParams) {
	...
	for k := range p.nbSubfr { //nolint:varnamelen // k indexes the subframe.
		aQ12 := p.predCoefQ12[((k>>1)|(1-lsfInterpFlag))*maxPredictLPCOrder:]
