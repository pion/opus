...
`-race` uses `baseline-arm64-race.json.gz`. The non-arm64 fallback is a strict
byte-exact comparison, not a tolerance. A future compiler or architecture that
changes decoder rounding must add a base-built golden rather than relax hashes.
